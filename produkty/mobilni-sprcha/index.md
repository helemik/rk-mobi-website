---
layout: landing
title: Mobilní sprcha
description: 'Moderní přenosný sprchový kout s ohřívačem.<br />Vhodná pro stavbu nebo zahradu.'
image: /assets/images/mobilni-sprcha.jpg
permalink: /produkty/mobilni-sprcha/
nav-menu: false
---

<div id="main">

<section id="one">
	<div class="inner">
		<header class="major">
			<h2>Mobilní sprcha</h2>
		</header>
		<p>Moderní mobilní sprcha vybavená průtokovým ohřívačem a umyvadlem s baterií. Vyrobena z ABS plastu, odolného vůči povětrnostním podmínkám, elegantní design v kombinaci šedé a černé barvy. Vypouklá střecha z polyethylenu. Protiskluzová podlaha, dveře s okénkem, pevné bezpečné zavírání.</p>
		<p>Rozměry: v 223 cm, ø 106 cm, h 106 cm. Hmotnost 77 kg. Odtokový vývod pro možné připojení ke kanalizaci.</p>
		<p>Praktický přenosný sprchový kout zajistí hygienu a komfort na vaší stavbě nebo zahradě.</p>
	</div>
</section>

<section id="two" class="spotlights">
	<section>
		<a href="{{ '/assets/images/produkty/1468_4-img20210319111656.jpg' | relative_url }}" class="glightbox image" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Mobilní sprcha - celkový pohled">
			<img src="{{ '/assets/images/produkty/1468_4-img20210319111656.jpg' | relative_url }}" alt="Mobilní sprcha" data-position="center center" />
		</a>
		<div class="content">
			<div class="inner">
				<header class="major">
					<h3>Cena</h3>
				</header>
				<p><strong>Cena 16.000 Kč bez DPH (19.360 Kč včetně DPH)</strong></p>
				<p>Dopravu mohu zajistit, cena dle individuální dohody.</p>
			</div>
		</div>
	</section>
	<section>
		<a href="{{ '/assets/images/produkty/1469_4-img20210319111707.jpg' | relative_url }}" class="glightbox image" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Mobilní sprcha - detail instalace">
			<img src="{{ '/assets/images/produkty/1469_4-img20210319111707.jpg' | relative_url }}" alt="Mobilní sprcha detail" data-position="center center" />
		</a>
		<div class="content">
			<div class="inner">
				<header class="major">
					<h3>Máte zájem?</h3>
				</header>
				<p>Kontaktujte nás pro více informací nebo cenovou nabídku na míru.</p>
				<ul class="actions">
					<li><a href="{{ "/kontakt/" | relative_url }}" class="button">Kontaktujte nás</a></li>
					<li><a href="{{ "/produkty/" | relative_url }}" class="button">« Zpět na produkty</a></li>
				</ul>
			</div>
		</div>
	</section>
</section>

<section id="three">
	<div class="inner">
		<header class="major">
			<h2>Fotogalerie</h2>
		</header>
		<div class="box alt">
			<div class="row uniform">
				<div class="2u"><span class="image fit"><a href="{{ '/assets/images/produkty/1470_4-img20210319111724.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Detail sprchovací hlavice"><img src="{{ '/assets/images/produkty/1470_4-img20210319111724.jpg' | relative_url }}" alt="Detail sprchovací hlavice" /></a></span></div>
				<div class="2u"><span class="image fit"><a href="{{ '/assets/images/produkty/1471_4-img20210319111736.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Ovládání a nastavení"><img src="{{ '/assets/images/produkty/1471_4-img20210319111736.jpg' | relative_url }}" alt="Ovládání a nastavení" /></a></span></div>
				<div class="2u"><span class="image fit"><a href="{{ '/assets/images/produkty/1472_4-img20230619093524.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Instalace v exteriéru"><img src="{{ '/assets/images/produkty/1472_4-img20230619093524.jpg' | relative_url }}" alt="Instalace v exteriéru" /></a></span></div>
				<div class="2u"><span class="image fit"><a href="{{ '/assets/images/produkty/1473_4-img20230622193017.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Praktické použití"><img src="{{ '/assets/images/produkty/1473_4-img20230622193017.jpg' | relative_url }}" alt="Praktické použití" /></a></span></div>
				<div class="2u"><span class="image fit"><a href="{{ '/assets/images/produkty/1474_4-img20230622193021.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Detail konstrukce"><img src="{{ '/assets/images/produkty/1474_4-img20230622193021.jpg' | relative_url }}" alt="Detail konstrukce" /></a></span></div>
				<div class="2u$"><span class="image fit"><a href="{{ '/assets/images/produkty/1475_4-img20230622193024.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Celkový pohled na instalaci"><img src="{{ '/assets/images/produkty/1475_4-img20230622193024.jpg' | relative_url }}" alt="Celkový pohled na instalaci" /></a></span></div>
				<div class="2u$"><span class="image fit"><a href="{{ '/assets/images/produkty/1476_4-img20230622193032.jpg' | relative_url }}" class="glightbox" data-gallery="mobilni-sprcha-gallery" data-glightbox="title: Detail připojení"><img src="{{ '/assets/images/produkty/1476_4-img20230622193032.jpg' | relative_url }}" alt="Detail připojení" /></a></span></div>
			</div>
		</div>
	</div>
</section>

</div>
